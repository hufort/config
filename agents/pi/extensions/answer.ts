	...
	try {
		...
		if (parsed && Array.isArray(parsed.questions)) {
			const questions = parsed.questions
				.filter((q: unknown): q is ExtractedQuestion => {
			...
			const extractionResult = await ctx.ui.custom<ExtractionResult | null>((tui, theme, _kb, done) => {
				...
				const doExtract = async () => {
					...
					const response = await complete(
						...
						{ apiKey: auth.apiKey, authToken, headers: auth.headers, signal: loader.signal },
					);

					if (response.stopReason === "aborted") {
						ctx.ui.notify("Question extraction aborted by model/provider", "error");
						return null;
					}
					...
					if (parsed === null) {
						...
						const errorMessage = response.errorMessage ? ` error=${response.errorMessage}` : "";
						ctx.ui.notify(
							`Question extraction returned invalid JSON: stopReason=${response.stopReason} contentTypes=${contentTypes}${errorMessage} text=${responseText.slice(0, 500)}`,
							"error",
						);
					}
					return parsed;
				...
				doExtract()
					...
					.catch((error) => {
						...
						ctx.ui.notify(`Question extraction failed: ${message}`, "error");
						done(null);
					});
